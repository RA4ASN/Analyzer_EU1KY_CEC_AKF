...
void SetCursor(void)
{
    ...
    x = x_0 + (CurPos)*23;
    if (x > 23)
        LCD_FillRect((LCDPoint){x - 23, 46}, (LCDPoint){x + 46, 49}, LCD_BLACK); //delete the old cursor
    else
        LCD_FillRect((LCDPoint){x, 46}, (LCDPoint){x + 46, 49}, LCD_BLACK); //delete the old cursor
    LCD_FillRect((LCDPoint){x, 46}, (LCDPoint){x + 23, 49}, LCD_WHITE);     // Set new cursor
}
...
void WriteText(void)
{
    ...
    {
        FONT_Write_N(FONT_FRANBIG, color, LCD_BLACK, x_0 + 23 * iCount, y_0, &txtbuf[iCount], 1);
    }
    SetCursor();
}
...
static void KeybHitCb(const TEXTBOX_t *tb)
{
    ...
    color = LCD_GREEN;
    LCD_FillRect((LCDPoint){x_0 + (CurPos)*23, y_0}, (LCDPoint){173 + (CurPos)*23, y_0 + 28}, LCD_BLACK); // delete the old character
    if (CurPos < maxlen - 1)
        CurPos++;
...
static void KeybHitBackspaceCb(void)
{
    ...
    color = LCD_GREEN;
    LCD_FillRect(LCD_MakePoint(x_0, y_0), LCD_MakePoint(350, 46), LCD_BLACK); //delete the old text
    WriteText();
}
...
static const TEXTBOX_t tb_keybal[] = {
    ...
    (TEXTBOX_t){.x0 = KBDX(7), .y0 = KBDY(3), .text = " ", .font = FONT_FRANBIG, .width = KEYW, .height = KEYH, .center = 1, .border = 1, .fgcolor = LCD_WHITE, .bgcolor = LCD_BLUE, .cb = (void (*)(void))KeybHitCb, .cbparam = 1, .next = (void *)&tb_keybal[42]},
    (TEXTBOX_t){
        .x0 = KBDX(7),
        .y0 = KBDY(4) + 4,
        .text = "OK",
...
uint32_t KeyboardWindow(char *buffer, uint32_t max_len, const char *header_text)
{
    ...
    LCD_FillRect(LCD_MakePoint(x_0, y_0), LCD_MakePoint(350, 49), LCD_BLACK);
